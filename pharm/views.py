...
def items(request):
    ...
    if request.method == 'POST':
        ...
        if form['export_to_csv'].value() == True:
            ...
            return response
    context={'title':title,'form':form,'queryset':queryset,'posts':posts}
...
def issue_item(request,pk):
    ...
    if form.is_valid():
        ...
        messages.success(request, 'item(s) has been sold successfully! ' + str(instance.quantity_of_packs) + " " + str(instance.item_name)+ ' now left in the store.')
        instance.save()

        return redirect('/detail/'+str(instance.id))
    
    context = {'title':'Selling '+str(queryset.item_name),'form':form,'queryset':queryset,'username':'issued by '+str(request.user)}
...
def receive_item(request,pk):
    ...
    if form.is_valid():
        ...
        #instance.issued_by_internally = str(request.user)
        instance.save()
        messages.success(request, 'stock added successfully! ' + str(instance.quantity_of_packs) + " " + str(instance.item_name)+ ' now in the store.')
